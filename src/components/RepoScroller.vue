...
<template>
    <div class="flex h-96 w-full flex-col items-center rounded-3xl bg-gray-100 p-8 ">
        <span class="flex w-full justify-start font-rubik text-sm text-blue-500 mb-1">Public Repository</span>
            ...
            <div class="flex w-full flex-row items-center justify-between">
                ...
                <div v-if="isReady" class="w-2/3" ref="scroller">
                    <div v-if="state[currentScrollIndex]" class="flex flex-col ">
                        <h3 class="font-lato text-6xl font-bold mb-8">
                            {{ state[currentScrollIndex].name }}
                        </h3>
                        <p class="max-h-20 overflow-hidden whitespace-normal font-rubik text-lg mb-16">
                            {{ state[currentScrollIndex].description || "" }}
                        </p>
                        <div>
                            <a class="rounded-2xl bg-gray-950 px-16 py-4 font-rubik text-lg font-medium text-white"
                                :href="state[currentScrollIndex].html_url" target="_blank">View Repo</a>
                        </div>
                    </div>
